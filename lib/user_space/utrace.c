...
{
	if (m0_pid_cached) {
		m0_error_printf("motr: can't set trace buffer size after it's"
				" been initialized\n");
		return -EBUSY;
	}

	if (size == 0 || !m0_is_po2(size) || size % m0_pagesize_get() != 0) {
		m0_error_printf("motr: incorrect value for trace buffer size"
...
#define CMDLINE "/proc/self/cmdline"
	...
	if (read_bytes == -1) {
		warn("motr: failed to read " CMDLINE);
	} else {
		if (read_bytes == sizeof(tbh->tbh_cli_args)) {
			warnx("motr: command line args don't fit into trace "
	...
	rc = close(cmdline_fd);
	if (rc == -1)
		warn("motr: failed to close " CMDLINE ": errno=%d", errno);
#undef CMDLINE
				   ...
				   const struct m0_trace_buf_header *tbh)
{
	...
	fprintf(ofile, "  motr_version:       %s\n", tbh->tbh_motr_version);
	fprintf(ofile, "  motr_git_describe:  %s\n", tbh->tbh_motr_git_describe);
	fprintf(ofile, "  motr_kernel:        %s\n", tbh->tbh_motr_kernel_ver);
...
static int mmap_m0tr_ko(const char *m0tr_ko_path, void **ko_addr)
{
	...
	kofd = open(m0tr_ko_path, O_RDONLY);
	if (kofd == -1) {
		warn("failed to open '%s' file", m0tr_ko_path);
		return EX_NOINPUT;
	}
	...
	*ko_addr = mmap(NULL, ko_stat.st_size, PROT_READ, MAP_PRIVATE,
	...
	if (*ko_addr == MAP_FAILED) {
		warn("failed to mmap '%s' file", m0tr_ko_path);
		return EX_OSERR;
	}
				   ...
				   ptrdiff_t *td_offset)
{
	if (tbh->tbh_buf_type == M0_TRACE_BUF_USER) {
		*td_offset = (char*)m0_trace_magic_sym_addr_get() -
		...
		rc = mmap_m0tr_ko(m0tr_ko_path, &ko_addr);
		if (rc != 0)
			return rc;
			       ...
			       unsigned int magic_symbols_nr)
{
	...
	rc = calc_trace_descr_offset(tbh, m0tr_ko_path, &td_offsets[0]);
	if (rc != 0)
		return rc;
	...
	if (magic_symbols != NULL) {
		...
		if (rc != 0)
			 ...
			 * symbols information, using only internal (libmotr)
			 * magic symbol */
			td_offsets_nr = 1;
	...
	while (!feof(trace_file)) {
		...
		do {
			...
			if (nr != sizeof trh.trh_magic) {
				...
				if (invalid_td_count > 0)
					warnx("Total number of unknown trace"
		...
		if (td->td_magic != M0_TRACE_DESCR_MAGIC) {
			if (invalid_td_count == 0)
				warnx("Invalid trace descriptor - most probably"
				      "the trace file was produced by a"
				      "different version of Motr");
			++invalid_td_count;
			continue;
		}
